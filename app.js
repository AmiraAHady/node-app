...
// path:/product/:id ,method:get

server.get("/product/:id", (req, res) => {
  var productId = +req.params.id;
  var selectedProduct = listOfPRoducts.find((prod) => prod.id == productId);
  if (selectedProduct) {
    res.send(selectedProduct);
  } else {
    res.send(`No Product with this id:${productId}`);
  }
});

// path:/product/:id ,method:delete

server.delete("/product/:id", (req, res) => {
  var productId = +req.params.id;
  try {
    var ListAfterDelete = listOfPRoducts.filter((prod) => prod.id != productId);
    console.log(ListAfterDelete);
    res.send("product Deleted successfully");
  } catch (error) {
    res.send("Cant delete product,someting went wrong!");
  }
});

// add Product api
// path:/addProduct ,method:post,
// {id:31,title:IPhone,price:1200,cateory:{id:12,name:electronics}}=>request body
//encrypt
// decrypt data using middleware
...
server.post("/addProduct", (req, res) => {
  try {
    ...
    res.send("Added to list successfully");
  } catch (error) {
    res.send("someting went wrong!");
  }
});

// path:/product/:id ,method:put

server.put("/product/:id", (req, res) => {
  try {
    var data = req.body; //updated data
    var id = +req.params.id; //id for product
    ...
    res.json({ message: "updated sucssesfully" });
  } catch (error) {
